...
const int raja_reco = 34;
const int raja_ovi = 32;
...
unsigned char rajakytkin_suppilo;
unsigned char rajakytkin_reco;
unsigned char rajakytkin_ovi;
...
void hae_raja()
{
  rajakytkin_ovi = digitalRead(raja_ovi);
  
  digitalWrite(enable, enable_paalle);      //virtaa moottorille
  digitalWrite(dirPin, suppilolle);     //kohti rajakytkinta

  rajakytkin_suppilo = digitalRead(raja_suppilo);

  while (rajakytkin_suppilo == LOW && rajakytkin_ovi == HIGH)
  {
...
void kaanna_recolle()
{
  ...
  while (step_laskuri < rajalta_recolle && rajakytkin_reco == LOW && rajakytkin_ovi == HIGH)
  {
    digitalWrite(stepPin, HIGH);
    ...
    rajakytkin_ovi = digitalRead(raja_ovi);
    rajakytkin_reco = digitalRead(raja_reco);
...
void kaanna_suppilolle()
{
  ...
  nopeus = hidas;
  //while(rajakytkin_suppilo == HIGH)

  while (step_laskuri > 0 && rajakytkin_ovi == HIGH)
  {
    digitalWrite(stepPin, HIGH);
    delayMicroseconds(nopeus);
    digitalWrite(stepPin, LOW);
...
void hammeri_ylos()
{
  ...
  while (step_laskuri < hammerin_irrotuskulma && rajakytkin_ovi == HIGH)
  {
    digitalWrite(stepPin, HIGH);
    delayMicroseconds(nopea);
    digitalWrite(stepPin, LOW);
...
void loop() {
  ...
  if (rajakytkin_suppilo == LOW && manual_suppilo == LOW)  //manuaali-ajo KALIBROINTIIN
  {
  ...
  if (rajakytkin_reco == LOW && manual_reco == LOW)    //manuaali-ajo KALIBROINTIIN       MUUTETTU 9.12
  {
    for (int x = 0; x < 1000; x++)
    {
      digitalWrite(enable, enable_paalle);      //virtaa moottorille
...
rajakytkin_reco = digitalRead(raja_reco);
  ...
  if (harvest_button == LOW && rajakytkin_ovi == HIGH)
  {
    nayttotila = 2;
    oled();
    ...
    while (rajakytkin_ovi == HIGH && nayttotila == 3)
    {
      rajakytkin_ovi = digitalRead(raja_ovi);
      }
  }
